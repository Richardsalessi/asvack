<div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 flex flex-col items-center text-center">
    ...
    <div class="h-64 w-full mb-4 overflow-hidden relative">
        @if(!empty($producto->imagenes) && count($producto->imagenes) > 0)
            <img src="data:image/png;base64,{{ $producto->imagenes[0]->contenido }}" alt="Imagen de {{ $producto->nombre }}" class="object-contain w-full h-full" style="user-select: none;">
        @else
    ...
    <!-- Agregar al carrito -->
    <form action="{{ route('carrito.agregar', $producto->id) }}" method="POST" class="add-to-cart-form w-full">
        @csrf
