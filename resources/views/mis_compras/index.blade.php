            ...
            <table class="min-w-full text-sm">
                ...
                @forelse($ordenes as $o)
                    @php
                        ...
                        // Colecta posibles rutas desde relación imagenes
                        $imgsRaw = $product?->imagenes
                                    ? $product->imagenes->map(fn($im) => $im->url ?? $im->ruta ?? $im->path ?? null)->filter()->values()->all()
                                    : [];

                        // Fallbacks por si guardaste algo directo en el producto
                        if (empty($imgsRaw)) {
                        ...
                        // Placeholder inline (no hace peticiones -> nunca 404)
